  ...
  try {
    ...
    for (const item of items) {
      await connection.query(
        ...
        [tenant_id, mesa_id, item.id, item.quantidade, item.preco_unitario || item.preco_venda, item.subtotal]
      );
    }
...
router.put("/:id", authMiddleware, async (req: any, res) => {
  ...
  try {
    ...
    for (const item of items) {
      await connection.query(
        "INSERT INTO vendas_itens (tenant_id, venda_id, produto_id, quantidade, preco_unitario, subtotal) VALUES (?, ?, ?, ?, ?, ?)",
        [tenant_id, existingMesa.id, item.id, item.quantidade, item.preco_unitario || item.preco_venda, item.subtotal]
      );
    }
