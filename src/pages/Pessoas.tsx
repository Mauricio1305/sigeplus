...
export const Pessoas = () => {
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    if (res.ok) { 
      ...
      setNewPessoa({ nome: '', tipo_pessoa: 'cliente', cpf_cnpj: '', telefone: '', email: '', endereco: '', cidade: '', uf: '', ativo: true, razao_social: '', nome_fantasia: '', telefone_fixo: '', telefone_celular: '', numero: '', cep: '', data_aniversario: '' });
      fetchPessoas(); 
    }
  };
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white w-full max-w-lg rounded-3xl p-8 shadow-2xl">
            ...
            <form onSubmit={handleAdd} className="grid grid-cols-2 gap-4 max-h-[70vh] overflow-y-auto pr-2">
              ...
              <div>
                <label className="block text-xs font-bold text-slate-400 uppercase mb-1">Data de Aniversário</label>
                <input type="date" className="w-full px-4 py-2 rounded-xl border border-slate-200" value={newPessoa.data_aniversario ? newPessoa.data_aniversario.split('T')[0] : ''} onChange={e => setNewPessoa({...newPessoa, data_aniversario: e.target.value})} />
              </div>
              <div>
